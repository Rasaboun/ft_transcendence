"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.Lobby = void 0;
var uuid_1 = require("uuid");
var game_type_1 = require("../types/game.type");
var game_instance_1 = require("../game.instance");
var auth_type_1 = require("../../auth/types/auth.type");
var Lobby = /** @class */ (function () {
    function Lobby(server, options, lobbyManager) {
        this.server = server;
        this.options = options;
        this.lobbyManager = lobbyManager;
        this.id = (0, uuid_1.v4)();
        this.nbPlayers = 0;
        this.state = game_type_1.GameState.Stopped;
        //public         clients:        	Map<string, AuthenticatedSocket> = new Map<string, AuthenticatedSocket>();
        this.clients = new Map();
        this.gameInstance = new game_instance_1.GameInstance(this, options.mode);
    }
    Lobby.prototype.addClient = function (client) {
        this.clients.set(client.login, client.roomId);
        client.join(this.id);
        client.lobbyId = this.id;
        client.lobby = this;
        console.log("Client ".concat(client.login, " joined"), this.id);
        if (this.nbPlayers < 2) {
            this.gameInstance.addPlayer(client.login);
            this.nbPlayers++;
            if (this.nbPlayers == 1) {
                this.server.to(client.roomId).emit("waitingForOpponent");
            }
            else {
                this.gameInstance.sendReady();
            }
        }
        else {
            var data = this.getGameData();
            client.emit("spectateSuccess", data);
        }
    };
    Lobby.prototype.startGame = function () {
        if (this.state == game_type_1.GameState.Started)
            return;
        this.state = game_type_1.GameState.Started;
        this.gameInstance.resetRound();
        this.gameInstance.gameLoop();
        this.server.emit('activeGames', this.lobbyManager.getActiveLobbies());
    };
    Lobby.prototype.removeClient = function (client) {
        var _this = this;
        client.lobby = null;
        client.leave(this.id);
        this.clients["delete"](client.login);
        //this.gameInstance.stop();
        if (this.gameInstance.isPlayer(client.login)) {
            this.clients.forEach(function (user, id) {
                _this.clients["delete"](id);
            });
            this.nbPlayers = 0;
            this.state = game_type_1.GameState.Stopped;
            this.sendToUsers('gameStopped', "");
        }
    };
    Lobby.prototype.destroy = function () {
        return __awaiter(this, void 0, void 0, function () {
            var clientSockets;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.gameInstance.stop();
                        this.nbPlayers = 0;
                        this.state = game_type_1.GameState.Stopped;
                        this.sendToUsers('gameStopped', "");
                        this.clients.forEach(function (user, id) {
                            _this.clients["delete"](id);
                        });
                        return [4 /*yield*/, this.server["in"](this.id).fetchSockets()];
                    case 1:
                        clientSockets = _a.sent();
                        clientSockets.forEach(function (socket) {
                            socket.leave(_this.id);
                        });
                        this.lobbyManager.destroyLobby(this.id);
                        this.server.emit('activeGames', this.lobbyManager.getActiveLobbies());
                        return [2 /*return*/];
                }
            });
        });
    };
    Lobby.prototype.playersId = function () { return this.gameInstance.playersId(); };
    Lobby.prototype.sendUpdate = function (event, data) { this.server.to(this.id).emit(event, data); };
    Lobby.prototype.needUpdate = function (event, data) {
        var firstClient = this.clients.keys()[0];
        this.server.to(firstClient).emit(event, data);
    };
    Lobby.prototype.sendToUsers = function (event, data) {
        return __awaiter(this, void 0, void 0, function () {
            var _this = this;
            return __generator(this, function (_a) {
                this.clients.forEach(function (roomId, clientLogin) {
                    _this.server.to(roomId).emit(event, data);
                });
                return [2 /*return*/];
            });
        });
    };
    Lobby.prototype.playerMoved = function (playerLogin, newPos) {
        this.gameInstance.updatePlayer(playerLogin, newPos);
    };
    Lobby.prototype.getPlayer = function (clientLogin) {
        return this.gameInstance.getPlayer(clientLogin);
    };
    Lobby.prototype.getGameData = function () {
        return this.gameInstance.getGameData();
    };
    Lobby.prototype.getMode = function () {
        return this.options.mode;
    };
    Lobby.prototype.isClient = function (clientLogin) {
        return this.clients.get(clientLogin) === undefined ? false : true;
    };
    Lobby.prototype.isPrivate = function () { return this.options.inviteMode; };
    return Lobby;
}());
exports.Lobby = Lobby;
